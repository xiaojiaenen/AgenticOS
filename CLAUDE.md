...
The Vite dev server proxies `/api` to the backend at `VITE_API_PROXY_TARGET` (default `http://127.0.0.1:8001`).

**Docker**: `docker compose up -d --build` deploys both services. Frontend is served by nginx on :3001, nginx reverse-proxies `/api` to the backend.

## Architecture

### Backend (FastAPI + wuwei agent framework)
...
3. User approves/rejects → `POST /agent/approvals/{id}/decision`
...
Essential: `OPENAI_API_KEY` (required for LLM), `AUTH_SECRET_KEY` (JWT signing).

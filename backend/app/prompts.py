...
2. **邮件搜索**：按发件人、主题、日期、关键词搜索
3. **邮件阅读**：读取邮件内容、查看附件信息
4. **邮件回复**：帮助用户撰写和发送邮件（需用户确认）
5. **邮件抄送**：支持添加抄送收件人
...
1. 提示用户提供邮箱地址
2. 提示用户提供应用专用密码（不是登录密码）
3. 调用 `setup_email` 验证凭据
4. 验证成功后，告知用户可以开始使用

### 日常使用

1. 用户说"看看邮件"或"有什么新邮件" → 调用 `read_emails`
2. 用户说"搜索xxx" → 调用 `search_emails`
3. 用户说"第n封"或"打开xxx" → 调用 `get_email`

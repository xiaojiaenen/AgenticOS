"""
SVG 质量检查工具 — 检查 SVG 文件是否符合 PPT 技术规范。
...
class SVGQualityChecker:
    ...
    def check_svg_string(
        self, svg_content: str, name: str = "slide", expected_format: str = "ppt169"
    ) -> Dict:
        """直接从 SVG 字符串检查质量，无需写入磁盘。

        将 SVG 内容写入临时文件后调用 check_file()，然后删除临时文件。
        返回与 check_file() 相同的 result 字典。
        """
        import tempfile
        import os
        ...
        try:
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".svg", prefix=f"{name}_", delete=False, encoding="utf-8"
            ) as f:
                f.write(svg_content)
                tmp_path = f.name
            result = self.check_file(tmp_path, expected_format)
            # Override the file name to use the logical name
            result["file"] = name
            return result
        finally:
...
def check_svg_quality(
    svg_content: str,
    name: str = "slide",
    expected_format: str = "ppt169",
) -> Dict:
    """便捷函数：一步检查 SVG 字符串质量。

    返回 check_file() 结果字典，包含 errors、warnings、passed 等字段。
    """

"""
SVG 图表坐标计算工具 — 为柱状图、饼图、折线图、雷达图和网格布局
提供精确的 SVG 像素坐标计算。
"""
...
def calculate_positions(
    ...
    """统一的图表坐标计算入口，返回格式化的坐标表字符串。

    chart_type: bar | pie | line | radar | grid
    data: "标签1:值1,标签2:值2,..." 或 "x1:y1,x2:y2" (line 模式)
    """
    ...
    chart_area = None
    if area:
        parts = parse_tuple(area)
        chart_area = ChartArea(parts[0], parts[1], parts[2], parts[3])

    if chart_type == "bar":
        coord = CoordinateSystem(format, chart_area)
        calc = BarChartCalculator(coord)
        d = parse_data_string(data)
        bw = bar_width if bar_width is not None else 50
        positions = calc.calculate(d, bar_width=bw, gap_ratio=gap_ratio,
                                   horizontal=horizontal)
        lines = [
            f"=== Bar Chart ({'Horizontal' if horizontal else 'Vertical'}) ===",
            f"Canvas: {CANVAS_FORMATS.get(format, {}).get('dimensions', format)}",
            f"Bars: {len(positions)}, Bar Width: {bw:.0f}px, Gap Ratio: {gap_ratio}",
            "",
        ...
        center_xy = parse_tuple(center) if center else (420, 400)
        r = radius if radius is not None else 200
        ir = inner_radius if chart_type == "donut" and inner_radius == 0 else inner_radius
        calc = PieChartCalculator(center_xy, r)
        d = parse_data_string(data)
        slices = calc.calculate(d, start_angle=start_angle, inner_radius=ir)
        return f"=== Pie Chart ===\n{calc.format_table(slices)}"

    elif chart_type == "radar":
        center_xy = parse_tuple(center) if center else (640, 400)
        r = radius if radius is not None else 200
        calc = RadarChartCalculator(center_xy, r)
        d = parse_data_string(data)
        points = calc.calculate(d)
        return f"=== Radar Chart ===\n{calc.format_table(points)}"

    elif chart_type == "line":
        coord = CoordinateSystem(format, chart_area)
        calc = LineChartCalculator(coord)
        xy_data = parse_xy_data_string(data)
        x_r = parse_tuple(x_range) if x_range else None
        y_r = parse_tuple(y_range) if y_range else None
        points = calc.calculate(xy_data, x_r, y_r)
        canvas_name = CANVAS_FORMATS.get(format, {}).get('dimensions', format)
        return f"=== Line Chart ===\nCanvas: {canvas_name}\n{calc.format_table(points)}"
        ...
        canvas_name = CANVAS_FORMATS.get(format, {}).get('dimensions', format)
        return f"=== Grid Layout ({rows}x{cols}) ===\nCanvas: {canvas_name}\n{calc.format_table(cells)}"

    else:
        raise ValueError(
            f"Unknown chart_type: {chart_type}. "
            f"Supported: bar, pie, donut, radar, line, grid"
        )

...
description: Website 设计品味核心规则——需求推断、三旋钮配置、反 AI 千篇一律指令、禁止模式、预检清单。Website 智能体生成网页时必读。详细参考见 references/taste-reference.md。
version: 2.0.0
tags: [website, design, taste, anti-slop]
when_to_use: Website 智能体生成网页时，加载此技能以获得高端设计规则
allowed_tools: [write_text_file, read_text_file, replace_text_in_file, build_website, load_skill_reference]
required_tools: []
---

# tasteskill: Anti-Slop Frontend Skill（核心规则）

> Landing pages, portfolios, and redesigns. Not dashboards, not data tables, not multi-step product UI.
> Every rule below is **contextual**. None of it fires automatically. First read the brief, then pull only what fits.
>
> **本文件为核心规则（~480 行）。** 详细参考（设计系统映射/架构约定/动效/性能/重新设计等）已移至 `references/taste-reference.md`，按需通过 `load_skill_reference` 加载。
...
以下章节已移至参考手册，按需加载：
- Section 2: 设计系统映射 → `load_skill_reference("references/taste-reference.md")`
- Section 3: 默认架构 → `load_skill_reference("references/taste-reference.md")`
- Section 5: 上下文感知主动行为 → `load_skill_reference("references/taste-reference.md")`
- Section 6: 性能与无障碍 → `load_skill_reference("references/taste-reference.md")`
- Section 7: 旋钮定义 → `load_skill_reference("references/taste-reference.md")`
- Section 8: 暗色模式 → `load_skill_reference("references/taste-reference.md")`
- Section 10: 参考词汇 → `load_skill_reference("references/taste-reference.md")`
- Section 11: 重新设计协议 → `load_skill_reference("references/taste-reference.md")`
- Section 12: Block Library → `load_skill_reference("references/taste-reference.md")`
- Appendix A/B/C → `load_skill_reference("references/taste-reference.md")`
...
**❌ 绝对禁止**：用 Unicode emoji（🎂、✦、🚀、💡 等）代替图标
- emoji 在不同平台渲染不一致，导出 PPTX 后可能显示异常
- emoji 无法控制颜色、大小、风格，破坏设计一致性

**✅ 正确做法**：所有装饰性图标必须使用 `<use data-icon="库名/图标名" .../>` 语法
```svg
<!-- ✅ 正确：使用图标系统 -->
<use data-icon="chunk-filled/rocket" x="100" y="200" width="48" height="48" fill="var(--accent)"/>

<!-- ❌ 错误：使用 emoji -->
<text x="100" y="200" font-size="48">🚀</text>
```

**降级规则**：如果 `search_icons` 返回"未初始化"或连续 2 次返回 0 结果，才可以用 `<text>` 元素代替（纯文字排版），但仍禁止使用 emoji。
...
**字号反差策略**：不要每页都用 28-40px 的"安全"标题。偶尔用大字号制造视觉冲击，但注意不要溢出容器：
- **数据页**：关键数字用 40-48px（在容器内），标签用 12px
- **章节页**：标题用 40-48px，副标题用 14px
- **封面**：主标题 48-60px，副标题 14-16px
- 规则：字号必须在容器边界内，溢出 = 任务失败
...
## 十三、布局变化规则（借鉴 ppt-master）

**默认每页都用对称网格布局 = "AI 生成感"的根源。** 必须有意识地变化。

### 11 种布局模式（策略师阶段选择）

| 模式 | 特征 | 适用场景 |
|------|------|---------|
| 单栏居中 | 所有元素居中对齐 | 封面/结尾/金句 |
| 对称分割 5:5 | 左右等宽 | 对比/优缺点 |
| 非对称分割 3:7 | 一侧主导 | 图文混排/案例 |
| 三栏并排 | 等宽三列 | 三个并列要点 |
| 四象限/矩阵 | 2×2 网格 | 分类/优先级矩阵 |
| 上下分割 | 上图下文或上文下图 | 全出血图片+说明 |
| Z 形/瀑布 | 左右交替叙事 | 故事线/流程 |
| 中心辐射 | 中心概念+周围节点 | 架构/关系图 |
| 全出血+浮动文字 | 图片占满，文字浮动 | 呼吸页/特色页 |
| 图文重叠 | 图片和文字交叠 | 英雄时刻/冲击页 |
| 负空间驱动 | 40-60% 留白，单元素 | 极简/高端感 |

### 变化规则

1. **相邻页布局模式不同**：如果第 3 页是对称分割，第 4 页不要也是对称分割
2. **同一 deck 至少使用 4 种不同模式**：8 页 deck 至少覆盖 4 种布局模式
3. **标题位置至少 3 种变化**：居中/左对齐/右对齐，不要每页都在 (80,110)
4. **卡片尺寸要有变化**：同类型的卡片在不同页面可以用不同尺寸
5. **呼吸页用负空间**：breathing 页用大量留白，不要用卡片网格
6. **阴影/边框/填充三选一**：每个容器只用一种装饰方式，不要叠加
7. **每页 accent 使用位置不同**：不要每页都在同一个位置放 accent 色
...
- 这页的布局模式与上一页不同吗？
- 标题位置与上一页不同吗？
- 如果是 breathing 页，有没有违规使用卡片网格？
- 这页至少有 2 个空间参数与上一页不同吗？

...
| 症状 | 可能原因 | 排查步骤 | 修复方案 |
|------|---------|---------|---------|
| `No space left on device` | 磁盘满 | 查 YARN 节点磁盘状态 | 清理日志/临时文件，扩容磁盘 |
...
| 症状 | 可能原因 | 排查步骤 | 修复方案 |
|------|---------|---------|---------|
| `ACCEPTED` 长时间不运行 | 队列资源不足 | 查调度器队列用量 | 调整队列容量 / 杀低优先级应用 |
| `FAILED` + `Application killed` | 超出资源限制 | 查应用详情和诊断信息 | 增大 `yarn.scheduler.maximum-allocation-mb` |
...
```
排查路径：
1. 查 Doris BE 节点列表 → 关注 Alive/Dead 状态和 TabletNum
...
| `No space left` | 磁盘满 | 查 HDFS 根目录用量 | 清理过期数据 / 扩容 DataNode |

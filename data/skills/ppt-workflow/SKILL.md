...
不要总是推荐同一个风格。从系统注入的 `CURRENT TIME` 行提取秒数（`HH:MM:SS` 中的 `SS`），用 `秒数 % 8` 选择主推荐风格，再加相邻两个作为备选：
...
示例：`CURRENT TIME: 2026-06-11 14:23:47 UTC` → 秒数 47 → 47 % 8 = 7 → 主推荐"瑞士国际"，备选"编辑墨水"+"现代极简"。

**⚠️ 必须把 3 个风格选项展示给用户，等用户选择后再继续。** 不要自己直接选一个。输出格式：

```
根据你的需求，推荐以下 3 种设计风格：
1. 🏔 瑞士国际 — 直角+hairline边框+16列网格，适合商业报告
2. 🖋 编辑墨水 — 衬线标题+大量留白，适合战略提案
3. ⚡ 现代极简 — 无衬线+大留白+细线条，适合产品发布
...
| 视觉焦点 | 每页有且仅有一个主导入口元素（字号最大或颜色最突出） | P1 |
| 层级清晰 | 眯起眼看，标题/正文/标签仍然可区分（字号差 >= 1.5x） | P1 |
| 留白充足 | 内容区域不超过画布的 60%，至少 40% 留白 | P1 |
| 布局变化 | 相邻页布局模式不同，同一 deck 至少 4 种布局 | P1 |

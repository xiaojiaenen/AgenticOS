    ...
    <AnimatePresence>
      {showSearch && (
        <motion.div
          ...
          className="fixed top-4 left-1/2 -translate-x-1/2 z-50 flex justify-center w-full max-w-lg px-4"
        >
          {isGlass ? (
            <LiquidGlass
              {...glassPresets.card}
            ...
            >
              ...
              <input
                ...
                className="bg-transparent border-none outline-none w-full text-white placeholder:text-gray-400 font-medium py-2"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />

              {searchQuery && (
                <div className="flex items-center gap-2 pr-2 border-r border-white/20 mr-2">
                  <span className="text-[10px] font-bold bg-white/10 text-gray-300 px-2.5 py-1 rounded-lg tabular-nums whitespace-nowrap">
              ...
              <button
                onClick={onClose}
                className="text-xs font-bold text-gray-400 hover:text-white px-3 py-1 bg-white/10 hover:bg-white/20 rounded-full transition-colors whitespace-nowrap"
                aria-label="关闭搜索"
              >
                取消
              </button>
            ...
            <div className="bg-white/80 backdrop-blur-3xl border border-white/60 shadow-md rounded-[2rem] px-6 py-2 flex items-center gap-3 w-full focus-within:ring-2 ring-sky-500/20 transition-all">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-sky-400">
                <circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>
              </svg>
              <input

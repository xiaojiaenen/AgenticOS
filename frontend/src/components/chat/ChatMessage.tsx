  ...
  return (
    ...
    >
      <motion.div whileHover={{ scale: 1.1, rotate: [0, -5, 5, 0] }}
        className={cn("w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm z-10 cursor-help transition-all",
          isGlass
            ? isUser ? "bg-white/20 text-white border border-white/30" : "bg-white/10 text-white border border-white/20"
      ...
      <div className={cn("flex flex-col gap-1", wideLayout ? "max-w-[78%]" : "max-w-[80%]", isUser ? "items-end" : "items-start")}>
        ...
        {shouldRenderBubble && (
          isGlass ? (
            <LiquidGlass
              {...glassPresets.card}
              tint={isUser ? "rgba(255,255,255,0.12)" : "rgba(255,255,255,0.06)"}
              radius={32}
              id={message?.id ? `bubble-${message.id}` : undefined}
              className={cn("relative group max-w-full min-w-0 overflow-hidden",
            ...
            {isTyping ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                <p className="whitespace-pre-wrap break-words leading-relaxed tracking-tight font-medium text-white"><HighlightedText text={message?.text || ''} counter={sessionCounter.current} searchQuery={searchQuery} activeMatchId={activeMatchId} messageId={message?.id} /></p>
              </div>
            ) : (
                ...
                {visibleText ? (
                  <ReactMarkdown remarkPlugins={[remarkGfm, ...(config.enableLaTeX ? [remarkMath] : [])]} rehypePlugins={[...(config.enableLaTeX ? [rehypeKatex] : [])]}
                    components={{
                      ...
                      h4: ({ children }) => <h4 className="text-white">{processChildren(children, sessionCounter.current, searchQuery, activeMatchId, message?.id)}</h4>,

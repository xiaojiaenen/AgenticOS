...
export const ChatSuggestions: React.FC<ChatSuggestionsProps> = ({ onSelect }) => {
  ...
  const suggestions = [
    { icon: <CodeIcon size={20} className={isGlass ? "text-blue-400" : "text-blue-500"} />, title: '创意写作', desc: '帮我写一个关于时空旅行的小说开头', text: '请帮我写一个关于时空旅行的小说开头，主角是一个在图书馆发现秘密通道的学生。' },
    { icon: <CodeIcon size={20} className={isGlass ? "text-purple-400" : "text-purple-500"} />, title: '编写代码', desc: '如何用 React 和 Tailwind 实现一个登录页面？', text: '请用 React 和 Tailwind CSS 编写一个美观、响应式的登录页面组件代码，并提供 HTML 预览。' },
    { icon: <RefreshIcon size={20} className={isGlass ? "text-green-400" : "text-green-500"} />, title: '数据分析', desc: '帮我制定一个下周的健身与饮食方案', text: '请根据我的情况，制定一个为期一周的科学健身计划和健康的饮食建议，目标是增肌减脂。' },
    { icon: <MascotCool size={20} className={isGlass ? "text-amber-400" : "text-amber-500"} />, title: '生活建议', desc: '推荐几本能提升幸福感的心理学书籍', text: '请推荐 5 本能提升生活幸福感、对抗焦虑的心理学书籍，并简述推荐理由。' }
  ];

  return (
    <div className="h-full flex flex-col items-center justify-center animate-in fade-in zoom-in duration-700">
      <div className="relative mb-8">
        <RandomMascot size={120} className={isGlass ? "opacity-60" : "opacity-80"} />
        <motion.div
          animate={{ y: [0, -4, 0] }}
          transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
          className={`absolute -top-6 -right-12 px-3 py-1 rounded-full shadow-sm text-xs font-bold flex items-center gap-1.5 ${
            isGlass
              ? "bg-white/10 border border-white/20 text-white backdrop-blur-sm"
              : "bg-white border border-sky-100 text-sky-500"
          }`}
        >
          <span>Hello!</span>
      ...
      <h1 className={`text-2xl font-bold mb-2 ${isGlass ? "text-white" : "text-slate-800"}`}>今天想聊些什么？</h1>
      <p className={`mb-8 max-w-sm text-center ${isGlass ? "text-gray-400" : "text-slate-500"}`}>你可以尝试点击下方的建议，或者直接在下方输入框告诉我你的想法。</p>
            ...
            >
              ...
              >
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className="font-bold text-white">{prompt.title}</span>
                </div>
                <p className="text-xs text-gray-400 line-clamp-1">{prompt.desc}</p>
              </LiquidGlass>
            </motion.div>
          ) : (
            <motion.button

...
export const AdminSidebar = React.memo(({ activeTab, setActiveTab, isMobile, isOpen, onClose }: AdminSidebarProps) => {
  ...
  return (
    <motion.aside
      ...
      className={cn(
        'relative z-20 flex h-full flex-shrink-0 flex-col overflow-hidden border-r border-slate-200/80 bg-[var(--admin-sidebar-bg)] shadow-[10px_0_36px_rgba(15,23,42,0.06)] ring-1 ring-white/50 backdrop-blur-2xl',
    ...
    >
                ...
                <div
                  className={cn(
                    ...
                    active
                      ...
                      : 'border-slate-200/80 bg-white/70 text-slate-500 group-hover:text-slate-700',
                  )}
                >
                  <item.icon size={18} />
                </div>
      ...
      <div className="relative z-10 space-y-3 border-t border-slate-200/80 p-4">
        <button
          ...
          className="flex w-full items-center gap-3 rounded-lg bg-zinc-900 px-3 py-2.5 text-sm font-bold text-white shadow-lg transition-all hover:-translate-y-0.5 hover:bg-zinc-800 hover:shadow-button active:translate-y-0 focus-visible:ring-2 focus-visible:ring-brand-400/60 focus-visible:ring-offset-2"
        >

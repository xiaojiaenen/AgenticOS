    ...
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <motion.div initial={{scale:0.95,y:20}} animate={{scale:1,y:0}} className="relative max-h-[85vh] w-full max-w-xl overflow-y-auto rounded-3xl border border-white/60 bg-white p-6 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          ...
          <Field label="描述"><textarea className="admin-input min-h-[60px] resize-y" value={draft.description} onChange={e=>setDraft({...draft,description:e.target.value})}/></Field>
          ...
          {draft.auth_type==="oauth2"&&(<>
            <Field label="OAuth Client ID"><input className="admin-input font-mono text-sm" value={draft.oauth_client_id||""} onChange={e=>setDraft({...draft,oauth_client_id:e.target.value})}/></Field>
            <Field label="OAuth Client Secret"><input className="admin-input font-mono text-sm" type="password" value={draft.oauth_client_secret||""} onChange={e=>setDraft({...draft,oauth_client_secret:e.target.value})}/></Field>
  ...
  return(
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <motion.div initial={{scale:0.95,y:20}} animate={{scale:1,y:0}} className="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-white/60 bg-white p-6 shadow-2xl">
        ...
        <div className="mt-5 space-y-4">
          <div className="grid grid-cols-2 gap-4"><Field label="名称" required><input className="admin-input font-mono text-sm" value={draft.name} onChange={e=>setDraft({...draft,name:e.target.value})} placeholder="search_issues"/></Field><Field label="展示名" required><input className="admin-input" value={draft.display_name} onChange={e=>setDraft({...draft,display_name:e.target.value})} placeholder="搜索工单"/></Field></div>
          <Field label="描述"><textarea className="admin-input min-h-[60px] resize-y" value={draft.description} onChange={e=>setDraft({...draft,description:e.target.value})}/></Field>
          <div className="grid grid-cols-2 gap-4"><Field label="方法"><select className="admin-input" value={draft.method} onChange={e=>setDraft({...draft,method:e.target.value})}>{METHODS.map(m=><option key={m} value={m}>{m}</option>)}</select></Field><Field label="路径" required><input className="admin-input font-mono text-sm" value={draft.path} onChange={e=>setDraft({...draft,path:e.target.value})} placeholder="/rest/api/2/issue/{id}"/></Field></div>
          <div className="grid grid-cols-2 gap-4"><Field label="超时(秒)"><input className="admin-input" type="number" value={draft.timeout_seconds} onChange={e=>setDraft({...draft,timeout_seconds:Number(e.target.value)})}/></Field><div className="flex items-end pb-1"><label className="flex items-center gap-2 text-sm font-bold text-slate-700"><input type="checkbox" checked={draft.requires_approval} onChange={e=>setDraft({...draft,requires_approval:e.target.checked})} className="rounded"/>需要审批</label></div></div>
          <div>

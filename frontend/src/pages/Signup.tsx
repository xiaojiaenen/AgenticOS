...
export const Signup = () => {
  ...
  return (
    <motion.div
      ...
      style={isGlass ? { background: '#000000' } : { background: 'linear-gradient(180deg, #def0f6 0%, #e7f4f9 28%, #e1f2f7 55%, #e3f2f7 100%)' }}
    >
      {isGlass ? (
        ...
        {(() => {
          const formPanel = (
            <>
              ...
              <h1 className={cn("text-3xl font-black tracking-tight", isGlass ? "text-white" : "text-slate-900")}>注册 AgenticOS</h1>
              <p className={cn("mt-2.5 text-sm font-medium", isGlass ? "text-white/50" : "text-slate-500")}>创建账号，开始你的智能协作</p>
            </div>

            <form className="space-y-5" onSubmit={handleSignup}>
              <Input label="昵称" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="请输入您的昵称" required autoComplete="name" />

              {/* 邮箱输入 + 补全 */}
              <div ref={emailRef} className="relative">
                <Input label="邮箱地址" type="email" value={email} onChange={(e) => { setEmail(e.target.value); setShowSuggestions(true); setSelectedSuggestionIdx(-1); }} onFocus={() => setShowSuggestions(true)} onKeyDown={handleEmailKeyDown} placeholder="you@example.com" required autoComplete="email" />
                <AnimatePresence>
                  {showSuggestions && suggestions.length > 0 && (
                    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                      className={cn("absolute left-0 right-0 top-full mt-1 rounded-xl border shadow-lg overflow-hidden z-50 backdrop-blur-xl", isGlass ? "bg-white/10 border-white/20" : "bg-white border-slate-200")}
                    >
                      {suggestions.map((s, i) => (
                        <button key={s} type="button" onMouseDown={(e) => { e.preventDefault(); selectSuggestion(s); }}
                          className={`w-full px-4 py-2.5 text-left text-sm transition-colors ${i === selectedSuggestionIdx ? 'bg-sky-50 text-sky-700' : isGlass ? 'text-white/70 hover:bg-white/10' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
              ...
              <div>
                <label htmlFor="signup-password" className={cn("mb-2 ml-1 block text-xs font-bold uppercase tracking-[0.15em]", isGlass ? "text-white/50" : "text-slate-500")}>密码</label>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className={cn("text-xs font-bold uppercase tracking-[0.15em] ml-1", isGlass ? "text-white/50" : "text-slate-500")}>邮箱验证码</label>
                  <Button type="button" variant="secondary" size="sm" onClick={handleSendCode} disabled={codeCountdown > 0} className="shrink-0 px-3 py-1 text-xs">
                    {codeCountdown > 0 ? `${codeCountdown}s` : '发送验证码'}
                  </Button>
                </div>
                <Input type="text" value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="输入 6 位验证码" maxLength={6} required />
                {codeSent && <p className="text-xs text-emerald-600 font-medium">验证码已发送到您的邮箱，请查收</p>}
              </div>

              {error && <div role="alert" className="rounded-2xl border border-rose-200 bg-rose-50/80 px-4 py-3 text-sm font-medium text-rose-700">{error}</div>}

    ...
    >
      ...
      >
        <div className="flex flex-col items-center mb-10">
          ...
          <p className="text-slate-500 mt-2.5 text-sm font-medium">加入 AgenticOS，开始你的智能协作</p>
        </div>

        <form className="space-y-6" onSubmit={handleSignup}>
          <div className="space-y-2">
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">昵称</label>
            <input 
              type="text" 
              value={name}
          ...
          <div className="space-y-2">
            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">密码</label>
            <input 
              type="password" 
              value={password}
        ...
        <div className="mt-10 text-center text-sm text-slate-500 font-medium">
          已有账号？ <a href="#" onClick={(e) => { e.preventDefault(); navigate('/login'); }} className="text-zinc-800 hover:text-zinc-900 font-bold underline decoration-zinc-300 underline-offset-4">立即登录</a>
        </div>
      </motion.div>
    </motion.div>
  );
